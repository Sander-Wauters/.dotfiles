...
dotfilesrepo="https://github.com/Sander-Wauters/.dotfiles.git"
aurhelper="yay"
repobranch="master"
export TERM=ansi
...
https://github.com/LukeSmithxyz/voidrice/commits/master.atom \"~dotfiles\""
...
usercheck() {
	! { id -u "$name" >/dev/null 2>&1; } ||
		whiptail --title "WARNING" --yes-button "CONTINUE" \
			--no-button "No wait..." \
			--yesno "The user \`$name\` already exists on this system. Installing for an existing user will OVERWRITE any conflicting settings/dotfiles on the user account.\\n\\nThis will NOT overwrite your user files, documents, videos, etc., so don't worry about that, but only click <CONTINUE> if you don't mind your settings being overwritten.\\n\\nNote also that this will change $name's password to the one you just gave." 14 70
}
...
trap 'rm -f /etc/sudoers.d/deploy-temp' HUP INT QUIT TERM PWR EXIT
echo "%wheel ALL=(ALL) NOPASSWD: ALL
Defaults:%wheel,root runcwd=*" >/etc/sudoers.d/deploy-temp
